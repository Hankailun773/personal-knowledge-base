...
18. **图片插入**：工具栏按钮（系统文件选择框）、拖拽到编辑器、剪贴板粘贴（Cmd+V）三种方式插入，图片以 base64 格式嵌入内容；点击图片后显示预设尺寸按钮（小 25% / 中 50% / 大 100% / 原始）及四角拖拽手柄，右下角手柄可自由缩放宽度，高度等比例跟随；预览模式下只读无手柄
19. **搜索关键词高亮样式**：搜索结果预览中关键词高亮改为彩色背景（浅色模式 #FFE066 黄色背景，深色模式 #7A6000 背景 + #FFE066 文字），替代原有加粗样式
20. **空分类引导提示**：当前选中的是普通分类且条目数为 0 时，列表区域中央显示两行引导文字（主文字"还没有条目" + 副文字"点击上方 + 按钮，新建第一个条目"）；最近编辑、标签、搜索为空时不受影响
...
{
  "categories": [
    { "id": "string", "name": "string", "icon": "string" }  // icon 为 emoji 字符串，默认空字符串
  ],
  "entries": [
    {
